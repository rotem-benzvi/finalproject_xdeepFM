...
from drive.MyDrive.RCSYS_finalproject.DeepCTRTorch.deepctr_torch.models.SEAttention import *

class SExDeepFM(BaseModel):
    ...
    def __init__(self, linear_feature_columns, dnn_feature_columns, dnn_hidden_units=(256, 256),
                 ...
                 dnn_activation='relu', dnn_use_bn=False, task='binary', device='cpu', gpus=None, attention_channels = 26):

        super(SExDeepFM, self).__init__(linear_feature_columns, dnn_feature_columns, l2_reg_linear=l2_reg_linear,
                                      l2_reg_embedding=l2_reg_embedding, init_std=init_std, seed=seed, task=task,
                                      device=device, gpus=gpus)

        self.se = SEAttention(channel=attention_channels,reduction=8).to(device)
    ...
    def forward(self, X):
        ...
        if self.use_cin:
            ...
            se_output = self.se(cin_input)
            cin_output = self.cin(se_output)
            cin_logit = self.cin_linear(cin_output)
        if self.use_dnn:
            if len(sparse_embedding_list) > 0 and len(dense_value_list) > 0:
                    sparse_input = torch.cat(sparse_embedding_list, dim=1)
                    se_sparse_output = self.se(sparse_input)
                    sparse_dnn_input = torch.flatten(se_sparse_output, start_dim=1)
                    dense_dnn_input = torch.flatten(torch.cat(dense_value_list, dim=-1), start_dim=1)
                    dnn_input = concat_fun([sparse_dnn_input, dense_dnn_input])
            elif len(sparse_embedding_list) > 0:
                dnn_input = torch.flatten(torch.cat(sparse_embedding_list, dim=-1), start_dim=1)
            elif len(dense_value_list) > 0:
                dnn_input = torch.flatten(torch.cat(dense_value_list, dim=-1), start_dim=1)
            else:
